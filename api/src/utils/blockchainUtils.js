...
const getCaUrl = async (orgName, ccp) => {
  ...
  if (!caURL) {
    throw new ApiError(
      ...
      "Invalid Certificate authority URL"
    );
  }
  return caURL;
};
...
const getCaInfo = async (orgName, ccp) => {
  ...
  if (!caInfo) {
    throw new ApiError(
      httpStatus.NOT_EXTENDED,
      "Invalid Certificate authority info"
    );
  }
  return caInfo;
};
...
const enrollAdmin = async (_ca, wallet, orgName, ccp) => {
  ...
  const enrollment = await ca.enroll({
    enrollmentID: config.caAdminId,
    enrollmentSecret: config.caAdminSecret,
  });
...
const registerUser = async (orgName, userName, department) => {
  ...
  console.log(
    `Secret for the user with userName: ${userName} -------> ${secret}`
  );
...
const getClient = async (ccp, orgName) => {
  const tlsCredentials = grpc.credentials.createSsl(
    Buffer.from(ccp.peers[`peer0.${orgName}.example.com`].tlsCACerts.pem)
  );

  return new grpc.Client(
    ccp.peers[`peer0.${orgName}.example.com`].url,
    tlsCredentials,
    {
      "grpc.ssl_target_name_override":
        ccp.peers[`peer0.${orgName}.example.com`].grpcOptions
