...
const imageUpload = upload.fields([{ name: 'invoice', maxCount: 1 }]);
...
exports.uploadFileToS3 = async (req, res, next) => {
  ...
  imageUpload(req, res, async (err) => {
    try {
      ...
      const { value: data, error } = prescriptionValidation.createPrescription.prefs({ errors: { label: 'key' }, abortEarly: false  }).validate(req.body)
      if (error) {
      ...
      if (req.body.isUpdate && !req.files?.length) {
        ...
        if (req.files.invoice) {
          let fileMetadata = await uploadFile(req.files.invoice[0], org);
