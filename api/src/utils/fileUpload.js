...
const getDataHash = (data) => {
  try {
    ...
    console.log(
      `Error occurred while creating file data hash: Error: ${error}`
    );
    return null;
  }
};
...
const imageUpload = upload.fields([{ name: "diagnosis", maxCount: 1 }]);
...
exports.uploadFileToS3 = async (req, res, next) => {
  ...
  imageUpload(req, res, async (err) => {
    try {
      ...
      const { value: data, error } = diagnosisValidation.createDiagnosis
      ...
      if (error) {
        ...
        return res
          ...
          .send(getSuccessResponse(httpStatus.BAD_REQUEST, errorMessage));
      }
      if (err) {
        logger.error({
          userInfo: req.loggerInfo,
          method: "uploadFileToS3",
          error: "Error in imageUpload : " + err,
        });
        ...
        return res
          ...
          .send(getSuccessResponse(httpStatus.FORBIDDEN, err.message));
      }
      if (req.body.isUpdate && !req.files?.length) {
        ...
        if (!files?.agreement?.length) {
          ...
          return res
            .status(httpStatus.FORBIDDEN)
            .send(
              getSuccessResponse(httpStatus.FORBIDDEN, "No files selected")
            );
        }
      ...
      return res
        ...
        .send(
          getSuccessResponse(httpStatus.INTERNAL_SERVER_ERROR, error.message)
        );
    }
  });
};
